...
export function Goal({
  ...
  const { data, error, isLoading, isFetching } = useResultQuery<GoalData>(
    type,
    {
      websiteId,
      startDate,
        ...
        <Grid gap>
          ...
          <Row alignItems="center" justifyContent="space-between" gap>
            <Row alignItems="center" gap>
              <Icon>
                {parameters.type === "path" ? <File /> : <Lightning />}
              </Icon>
          ...
          <Row alignItems="center" gap="6">
            ...
            <Text weight="bold" size="4xl">
              {data?.total
                ? Math.round((+data?.num / +data?.total) * 100)
                : "0"}
              %
            </Text>
          </Row>
